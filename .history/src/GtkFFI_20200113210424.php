...
abstract class GtkFFI
{
    ...
    final function checkDeclare($class, $name)
    {
        ...
        foreach (['ID', 'MATCH_PREFIX', 'MATCH_FULL', 'UNIMPLEMENT'] as $constant) {
            if ((new ReflectionClassConstant($class, $name))->getDeclaringClass()->name !== $class) {
                throw new RuntimeException("$class must declare '$constant' constant");
            }
        }
    }
