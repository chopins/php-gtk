...
abstract class GtkFFI
{
    ...
    {
        var_dump( (new ReflectionClassConstant($class, $name))->getDeclaringClass()->name, $name);
        return (new ReflectionClassConstant($class, $name))->getDeclaringClass()->name == $name;
    }

    private function initCDef($class)
    {
        if(!$this->checkDeclareConstant($class, 'ID')){
            throw new \CompileError("$class must declare 'ID' constant");
        }
